    ...
    return (
        ...
        {detailVideoGame && (detailVideoGame.createdInDb === true ? (
          <div className={styles.detailCard}>
            ...
            <h4>Released at: {detailVideoGame?.releaseDate ? detailVideoGame.releaseDate.slice(0, 10) : "Unknown"}</h4>
            <h4>Rating: {detailVideoGame?.rating}</h4>
            <h4>Description: <p dangerouslySetInnerHTML={{ __html: detailVideoGame?.description }}></p></h4>
            <h3>Platforms: {detailVideoGame.platforms?.map(el => el).join(' - ') || 'No platforms'}</h3>
            <h3>Genres: {detailVideoGame.genres?.map(el => el.name).join(' - ') || 'No genres'}</h3>
          ...
          <div className={styles.detailCard}>
            ...
            <h3>Platforms: {detailVideoGame.platforms?.map(el => el).join(' - ') || 'No platforms'}</h3>
            <h3>Genres: {detailVideoGame.genres?.map(el => el.name).join(' - ') || 'No genres'}</h3>
            <Link to="/home"><button className={styles.volverButton}>Back to Home</button></Link>
          </div>
        ))}
      </div>
